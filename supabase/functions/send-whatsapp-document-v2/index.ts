...
serve(async (req) => {
  ...
  try {
    ...
    const { messageId, mediaUrl, contactPhoneNumber, fileName, caption, replyId } = await req.json();
    ...
    console.log('📤 [send-whatsapp-document-v2] Sending document for message:', messageId);
    ...
    if (fileName) {
      uazapiPayload.fileName = fileName;
    }

    if (caption) {
      uazapiPayload.text = caption;
    }
    ...
    console.log('📤 Sending to UAZAPI:', { number: cleanPhone, fileName, hasCaption: !!caption });
    ...
    console.log('✅ Document sent successfully:', whatsappMessageId);
    ...
    console.error('❌ Error in send-whatsapp-document-v2:', error);
    return new Response(JSON.stringify({ 
      success: false, 
      error: error.message 
    }), {
