...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { messageId, mediaUrl, contactPhoneNumber, caption, replyId } = await req.json();
    ...
    console.log('📤 [send-whatsapp-image-v2] Sending image for message:', messageId);
    ...
    console.log('📤 Sending to UAZAPI:', { number: cleanPhone, hasCaption: !!caption });
    ...
    console.log('✅ Image sent successfully:', whatsappMessageId);
    ...
    console.error('❌ Error in send-whatsapp-image-v2:', error);
    return new Response(JSON.stringify({ 
      success: false, 
      error: error.message 
    }), {
