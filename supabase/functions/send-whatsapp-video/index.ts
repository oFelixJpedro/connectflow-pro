...
serve(async (req) => {
  console.log('\n')
  console.log('╔══════════════════════════════════════════════════════════════════╗')
  console.log('║              🎬 SEND-WHATSAPP-VIDEO - INICIADA                   ║')
  console.log('╚══════════════════════════════════════════════════════════════════╝')
  ...
  try {
    ...
    console.log(`   - text: ${text ? text.substring(0, 50) + '...' : '(sem legenda)'}`)
    console.log(`   - duration: ${duration || '(não informado)'}`)
    console.log(`   - quotedMessageId: ${quotedMessageId || '(nenhum)'}`)

    // Validate required fields
    if (!videoData || !fileName || !mimeType || !conversationId || !connectionId || !contactPhoneNumber) {
    ...
    // Validate mime type
    const allowedMimeTypes = ['video/mp4', 'video/avi', 'video/x-msvideo', 'video/quicktime', 'video/x-matroska', 'video/webm']
    if (!allowedMimeTypes.includes(mimeType)) {
      console.log(`❌ Erro: Tipo MIME não suportado: ${mimeType}`)
      return new Response(
        JSON.stringify({ success: false, error: 'Formato não suportado. Use MP4, AVI, MOV, MKV ou WebM.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    try {
      ...
      videoBuffer = new Uint8Array(binaryString.length)
      for (let i = 0; i < binaryString.length; i++) {
        videoBuffer[i] = binaryString.charCodeAt(i)
      }
      console.log(`✅ Base64 decodificado: ${videoBuffer.byteLength} bytes (${(videoBuffer.byteLength / 1024 / 1024).toFixed(2)} MB)`)
    } catch (e) {
      console.log(`❌ Erro ao decodificar base64: ${e}`)
      return new Response(
        JSON.stringify({ success: false, error: 'Invalid video data' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    if (videoBuffer.byteLength > maxSize) {
      console.log(`❌ Erro: Arquivo muito grande: ${videoBuffer.byteLength} bytes`)
      return new Response(
        JSON.stringify({ success: false, error: 'Arquivo muito grande. O limite é 100MB.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    const storagePath = `${connection.company_id}/${connectionId}/${yearMonth}/video_${timestamp}_${random}.${extension}`

    console.log(`📁 Storage path: ${storagePath}`)
    console.log(`⏳ Iniciando upload (pode demorar para vídeos grandes)...`)
    ...
    if (uploadError) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'Erro ao fazer upload do vídeo. Tente novamente.' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    if (uazapiSuccess) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: 'Erro ao enviar vídeo para o WhatsApp.',
          messageId: messageId,
          status: 'failed'
        }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
