...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // 5.5️⃣ AVALIAÇÃO AUTOMÁTICA (para ação 'close')
    // ═══════════════════════════════════════════════════════════════════
    if (action === 'close') {
      console.log('\n┌─────────────────────────────────────────────────────────────────┐')
      console.log('│ 5.5️⃣  DISPARANDO AVALIAÇÃO AUTOMÁTICA                           │')
      console.log('└─────────────────────────────────────────────────────────────────┘')
      
      // Fire-and-forget: dispara a avaliação sem bloquear a resposta
      const evaluateUrl = `${supabaseUrl}/functions/v1/evaluate-conversation`
      ...
      fetch(evaluateUrl, {
        ...
        .then(response => {
          console.log(`✅ [AUTO-EVAL] Avaliação disparada para conversa ${conversationId}, status: ${response.status}`)
        })
        .catch(error => {
          console.error(`⚠️ [AUTO-EVAL] Erro ao disparar avaliação para conversa ${conversationId}:`, error)
        })
      
      console.log('📤 Avaliação automática disparada (fire-and-forget)')
    }
